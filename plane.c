	...
	while (attrib_name[0] != '}') {
	   ...
	   /* fill in code here */
                if(strcmp(attrib_name, "normal") == 0){
                    count = fscanf(in,"%lf %lf %lf",&pln->normal.x, &pln->normal.y, &pln->normal.z);
                    assert(count == 3);
                    attrcount += 1;
                }

                else if(strcmp(attrib_name, "point") == 0){
                    count = fscanf(in,"%lf %lf %lf",&pln->point.x, &pln->point.y, &pln->point.z);
                    assert(count == 3);
                    attrcount += 1;
                }
...
void plane_print(object_t  *obj, FILE  *out)
{
   plane_t *pln = obj->priv;
    ...
    fprintf(out, "%-12s %5.1lf %5.1lf %5.1lf\n", "normal", pln->normal.x, pln->normal.y, pln->normal.z);
    fprintf(out, "%-12s %5.1lf %5.1lf %5.1lf\n", "point", pln->point.x, pln->point.y, pln->point.z);
...
double  plane_hits(object_t  *obj, vec_t  *base, vec_t  *dir)    
{
   ...
   /* Compute ndotb */
   ndotq = vec_dot(&pln->normal, &pln->point);
